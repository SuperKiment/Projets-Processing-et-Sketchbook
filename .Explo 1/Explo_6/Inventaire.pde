class Stockage {
  ...
  class Case {
    ...
    int coordCX, coordCY, quantite = 0;
    String objet = "";

    Case(int newcoordX, int newcoordY) {
      coordCX = newcoordX;
      coordCY = newcoordY;
      print("créé : Case d'inventaire / ");
    }
    ...
    void Affichage(int x, int y) {
      ...
      AffichageRect(x, y);
      fill(255);
      text(quantite, x + coordCX*50, y + coordCY*50+40);
      pop();
    }
    ...
    void AffichageRect(int x, int y) {
      ...
      rect(x + coordCX*50, y + coordCY*50, 40, 40);
    }
  }
...
Stockage inventaire = new Stockage(10, 4, 100, 100);
