  ...
  boolean CollisionBlocXOk() {
    ...
    for (Bloc bloc : TousBlocs) {
      if (posy == bloc.y) {
        ...
        if (distX >  0) if (x+mvtx < bloc.x + 0.5 + taille/2) noCollision = false;
        //if (distX == 0) aDroite = 0;
        if (distX <  0) if (x+mvtx > bloc.x - 0.4 - taille/2) noCollision = false;
  ...
  boolean CollisionBlocYOk() {
    ...
    for (Bloc bloc : TousBlocs) {
      if (posx == bloc.x) {
        ...
        if (distY >  0) if (y+mvty < bloc.y + 0.4 + taille/2) noCollision = false;
        //if (distY == 0) enBas = 0;
        if (distY <  0) if (y+mvty > bloc.y - 0.5 - taille/2) noCollision = false;
