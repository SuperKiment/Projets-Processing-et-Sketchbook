...
class Explosion {
  ...
  void Expension() {
    radius += (radiusMax-radius)/8;   //Baboum
  }

  boolean StopExplosion() {
    if (radiusMax - radius < 5) return true;   //Baboum ?
    else return false;
  }
...
void Explosions() {
  for (int i = 0; i<AllExplosions.size(); i++) {
    ...
    explosion.Fonctions();                    //Fonctions des explosions
...
void Explosion(float x, float y, int radius) {
  AllExplosions.add(new Explosion(x, y, radius));    //Fonction pour invoquer une explosion direct
